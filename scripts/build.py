...
def make_files(root=ROOT):
    ...
    files['ExosuitMultiSelect-manifest.json']=(json.dumps(provenance,ensure_ascii=False,indent=2)+'\n').encode()
    return files

def verify_files(files):
    provenance=json.loads(files['ExosuitMultiSelect-manifest.json'])
    expected=set(files)-{'ExosuitMultiSelect-manifest.json'}
